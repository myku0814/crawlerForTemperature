...
(async () => {
    ...
    await new Promise(resolve => {
        fs.readFile('info.txt', 'utf8', (err, data) => {
            if(err) throw err;
            files = [];
            data.split(/},\s*{/).forEach(ele => {
                files.push(Object.assign({}, {
                    station: ele.match(/station:\s*'[a-zA-Z0-9]+'/)[0].replace(/station:\s*'|'$/g,''),
                    stname: ele.match(/stname:\s*'.+'/)[0].replace(/stname:\s*'|'$/g,'')
                }));
            });
            resolve();
        });
    ...
    // custom settings
    const check = (str) => {
        const tmp = Number(str.replace(/^\s+|\s+$/g, '').replace(/^0+(?=[1-9]+)/, ''));
